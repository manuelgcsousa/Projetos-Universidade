    ...
    if (file.is_open()) {
        for (int i = 0; i < stacks; i++) {
            ...
            for (int j = 0; j < slices; j++) {
                ...
                file << sin(beta + deltaBeta) * sin(alpha + deltaAlpha) << " " << cos(beta + deltaBeta) << " " << sin(beta + deltaBeta) * cos(alpha + deltaAlpha) << "\n";
                file << (s + deltaS) << " " << (t - deltaT) << " " << -1 << "\n";
                ...
                file << sin(beta) * sin(alpha + deltaAlpha) << " " << cos(beta) << " " << sin(beta) * cos(alpha + deltaAlpha) << "\n";
                file << (s + deltaS) << " " << t << " " << -1 << "\n";
